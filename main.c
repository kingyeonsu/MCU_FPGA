...
#include "./driver/FPGA_FND/FPGA_FND.h"
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  Button ledButton;
  Button_GetInstance(&ledButton, GPIOC, GPIO_PIN_13);

  Button stopButton;
  Button_GetInstance(&stopButton, GPIOC, GPIO_PIN_2);

  Button resetButton;
  Button_GetInstance(&resetButton, GPIOC, GPIO_PIN_3);

  Led testLed;
  Led_GetInstance(&testLed, GPIOA, GPIO_PIN_5);
  ...
  while (1)
  {
	  if (Button_GetState(&ledButton) == RELEASED)
	  {
		  HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_4);
		  Led_Toggle(&testLed);
	  }
	  if (Button_GetState(&stopButton) == RELEASED)
	  {
		  stopWatchState = !stopWatchState;
	  }
	  if (Button_GetState(&resetButton) == RELEASED)
	  {
		  number = 0;
	  }
